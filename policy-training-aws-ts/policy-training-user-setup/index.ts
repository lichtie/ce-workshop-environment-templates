...
const workshopStack = new pulumi.StackReference("workshop-stack", { name: workshopStackRef });

const gitlabRepoUrl         = workshopStack.getOutput("gitlabRepoUrl").apply(v => String(v));
const awsEscEnvironmentName = workshopStack.getOutput("awsEscEnvironmentNameOut").apply(v => String(v));
const participantTeamName   = workshopStack.getOutput("participantTeamNameOut").apply(v => String(v));
const awsRoleArn            = workshopStack.getOutput("awsRoleArnOut").apply(v => String(v));
...
const gitlabProjectId = gitlabProjectIdOverride !== undefined
...
const extraStackTags         = config.getObject<Record<string, string>>("extraStackTags") ?? {};
...
const projectSlugs = ["s3-website", "lambda-api", "rds-database", "ec2-instance", "iam-policies"];
...
const userTeam = new pulumiservice.Team(`team-${username}`, {
    organizationName: org,
    teamType: "pulumi",
    name: `${username}-team`,
...
new pulumiservice.TeamRoleAssignment(`role-assignment-${username}`, {
    organizationName: org,
...
projectSlugs.forEach((slug, i) => {
    new pulumiservice.StackTags(`tags-${username}-${slug}`, {
        organization: org,
        project: slug,
        stack: username,
        tags: allStackTags,
...
projectSlugs.forEach((slug, i) => {
    new pulumiservice.TeamStackPermission(`stack-perm-${username}-${slug}`, {
        organization: org,
...
new pulumiservice.TeamEnvironmentPermission(`env-perm-${username}`, {
    organization: org,
    team: userTeam.name.apply((n) => n!),
    project: "default",
...
projectSlugs.forEach((slug, i) => {
    new pulumiservice.DeploymentSettings(`deploy-${username}-${slug}`, {
        organization: org,
        project: slug,
        stack: username,
        sourceContext: {
...
const stackDestroyAt = new Date(Date.now() + stackTtlDays * 86400_000).toISOString();

projectSlugs.forEach((slug, i) => {
    new pulumiservice.DeploymentSchedule(`stack-ttl-${username}-${slug}`, {
        organization: org,
        project: slug,
        stack: username,
...
const preventativeGroup = new pulumiservice.PolicyGroup(`preventative-${username}`, {
    organizationName: org,
...
const auditGroup = new pulumiservice.PolicyGroup(`audit-${username}`, {
    organizationName: org,
    name: `${username}-audit`,
    entityType: "stacks",
    mode: "audit",
...
const parentDestroyAt = new Date(Date.now() + parentStackTtlDays * 86400_000).toISOString();
